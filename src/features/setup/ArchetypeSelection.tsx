...
const ArchetypeSelection: React.FC<ArchetypeSelectionProps> = ({ onComplete }) => {
  ...
  return (
    <div className="flex-1 overflow-y-auto h-full relative scrollbar-hide bg-background-dark p-6">
      <div className="w-full max-w-[960px] mx-auto flex flex-col gap-8 md:gap-12 py-10">
          ...
          <div className="flex flex-col gap-4 text-center items-center animate-in fade-in slide-in-from-bottom-4 duration-700">
            <h1 className="text-3xl md:text-5xl font-black leading-tight tracking-tight max-w-2xl text-white">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full animate-in fade-in slide-in-from-bottom-8 duration-700 delay-150">
            ...
            <label className={`group relative flex flex-col gap-4 rounded-xl border-2 ${selectedArchetype === 'performance' ? 'border-primary bg-card-dark' : 'border-surface-dark bg-card-dark/50'} p-6 md:p-8 cursor-pointer hover:border-primary hover:shadow-xl hover:shadow-primary/5 transition-all duration-300`}>
              <input 
              ...
              <div className="flex flex-col gap-2">
                <h3 className="text-xl font-bold text-white">Performance Optimizer</h3>
            ...
            {/* Card 2: PCOS Manager */}
            <label className={`group relative flex flex-col gap-4 rounded-xl border-2 ${selectedArchetype === 'pcos' ? 'border-primary bg-card-dark' : 'border-surface-dark bg-card-dark/50'} p-6 md:p-8 cursor-pointer hover:border-primary hover:shadow-xl hover:shadow-primary/5 transition-all duration-300`}>
              <input 
                className="peer sr-only" 
                name="archetype" 
                type="radio" 
              ...
              <div className="flex flex-col gap-2">
                <h3 className="text-xl font-bold text-white">PCOS Manager</h3>
          ...
          <div className="flex flex-col items-center gap-6 animate-in fade-in slide-in-from-bottom-8 duration-700 delay-300">
            <button 
                ...
                className="w-full max-w-[480px] h-14 flex items-center justify-center rounded-xl bg-primary disabled:opacity-50 disabled:cursor-not-allowed text-white text-lg font-bold shadow-lg hover:bg-primary/90 hover:scale-[1.02] active:scale-[0.98] transition-all"
            >
              Start My Journey
            </button>
            <div className="flex flex-col items-center gap-2">
              <div className="flex items-center gap-2 text-text-secondary text-xs font-medium bg-surface-dark px-3 py-1.5 rounded-full">
                <span className="material-symbols-outlined text-[16px]">lock</span>
                <span>Your data is encrypted and stored locally.</span>
              </div>
              <a className="text-text-secondary text-sm font-normal underline hover:text-primary transition-colors" href="#">
